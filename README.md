...
Replace `<REPO_URL>` with your GitHub repository (main branch). The blueprint uses `render.yaml` with Node runtime, `npm ci && npm run build`, and `npm run start`. See [docs/DEPLOY_RENDER.md](docs/DEPLOY_RENDER.md) for env vars and post-deploy checks.
